...
async def start(_: Client, msg: Message):
    try:
        try:
            referral_id = int(msg.command[1])
        except (ValueError, IndexError):
            await user.inset_user(shared.database, msg.from_user.id,
                                  msg.from_user.first_name, msg.from_user.is_premium)
        else:
            try:
                ...
                await user.update_referral_count(shared.database, referral.telegram_id, referral.referrals_count + 1)
                await user.inset_user(shared.database, msg.from_user.id,
                                      msg.from_user.first_name, msg.from_user.is_premium, referral_id)
        ...
        if current_user.phone:
            ...
            return await msg.reply_text(current_user.user_status(),
                                        reply_markup=buttons.request_contact_button("📱Telefon raqamini yuborish"))
...
async def my_id(_: Client, msg: Message):
    return await msg.reply_text(
        f"{msg.from_user.first_name} sizning ID  raqamingiz 🆔<code>{msg.from_user.id}</code>\n\n"
